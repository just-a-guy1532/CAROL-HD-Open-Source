...
class PlayState extends MusicBeatState
{
	...
	if(dialogueList.contains(daigleufggeeNAEMMEA)){
		...
		kadeEngineWatermark = new FlxText(4,healthBarBG.y + 60 ,0,SONG.song + " - " + CoolUtil.difficultyFromInt(storyDifficulty) + (Main.watermarks ? " | KE " + MainMenuState.kadeEngineVer : ""), 16);
		...
		scoreTxt = new FlxText(FlxG.width / 2 - 235, healthBarBG.y + 60, 0, "", 20);
			...
			if (camFollow.x != dad.getMidpoint().x + 150 && !PlayState.SONG.notes[Std.int(curStep / 16)].mustHitSection)
			{
				...
				switch (dad.curCharacter)
				{
					...
					case 'senpai-angry':
						...
						camFollow.x = dad.getMidpoint().x - 100;
					case 'hellchart-carol':
						camFollow.y = dad.getMidpoint().y - 560;
						camFollow.x = dad.getMidpoint().x - 100;
				}
