...
class StoryMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for(i in 0...7){
			FlxG.sound.cache(Paths.music('type' + i));
		}

		FlxG.sound.playMusic(Paths.music('fuck-the-pis-lol'), 1);
		 
		synth = new FlxSound().loadEmbedded(Paths.music('type'),true);
		drums = new FlxSound();
		synth.play();
	...
	function changeWeek(change:Int = 0):Void
	{
		...
		if(curWeek > 0){
			drums.loadEmbedded(Paths.music('type' + curWeek),true);
			drums.volume = 0;
			drums.play();
			drums.fadeIn(0.5);
